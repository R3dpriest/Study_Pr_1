...
if($_SERVER['REQUEST_METHOD']==='POST'){
	if(isset($_POST['CMS_Sel_Fo2'])&&strlen($_POST['CMS_Sel_Fo2'])>=1){
			...
			$CMS_Content4c=$CMS_Content4b->fetchAll(PDO::FETCH_ASSOC);
		foreach($CMS_Content4c as $U1b){
			$CMS_Sel_Fo2_data.='<div class="CMS_EDI_Sub_item"><div class="CMS_EDI_Sub_item_Txt"><div>Label: '.$U1b['lang_label'].'</div></div><div class="CMS_EDI_Sub_item_In"><div class="CMS_Lo"><div id="CMS_EDI_Foi'.$U1b['id'].'">ID</div><div id="CMS_EDI_Eoi'.$U1b['id'].'">'.$U1b['id'].'</div></div><div class="CMS_Lo"><div id="CMS_EDI_Fofi'.$U1b['id'].'">This form</div><div id="CMS_EDI_Eofi'.$U1b['id'].'">'.$U1b['field_id'].'</div></div><div class="CMS_Lo"><div id="CMS_EDI_Foi'.$U1b['id'].'">Language</div><div id="CMS_EDI_Eoi'.$U1b['id'].'">'.$U1b['lang_id'].'</div></div><div class="CMS_Lo"><div id="CMS_EDI_Foi'.$U1b['id'].'">Word used as a background text</div><div id="CMS_EDI_Eoi'.$U1b['id'].'">'.$U1b['lang_placeholder'].'</div></div><div class="CMS_Lo"><div id="CMS_EDI_Foi'.$U1b['id'].'">Text seen inside buttons</div><div id="CMS_EDI_Eoi'.$U1b['id'].'">'.$U1b['lang_center'].'</div></div></div>';
		}
		echo $CMS_Sel_Fo2_data;
	}
}
	...
	if(isset($_POST['CMS_Sel_Fo3'])&&strlen($_POST['CMS_Sel_Fo3'])>=1){
		...
		foreach($CMS_Content6c as $U3a){
			foreach($GrabTypes3 as $PageType){
				...
				$CMS_Sel_Fo3_data.='<div class="CMS_EDI_Sub_item"><div class="CMS_EDI_Sub_item_Txt"><div placeholder="'.$Type_Desc.'">Type: '.$Type.'</div><div><input type="number" id="CMS_EDI_Sort'.$U3a['id'].'" value="'.$U3a['sort'].'"></div></div>';
				switch($U3a['Page_types_id']){
					case "0": case "2":
						$CMS_Deep_Grab1a="SELECT * FROM CMS_content_raw WHERE id=:CMS_Raw_Id";
						$CMS_Deep_Grab1b=$SQL_Con->prepare($CMS_Deep_Grab1a);
						$CMS_Deep_Grab1b->execute([':CMS_Raw_Id'=>$U3a['Content_id']]);
						$CMS_Deep_Grab1c=$CMS_Deep_Grab1b->fetch(PDO::FETCH_ASSOC);
						$CMS_Sel_Fo3_data.='<div class="CMS_EDI_Sub_item_In"><div class="CMS_Lo"><textarea id="CMS_Raw_Con_'.$U3a['Page_types_id'].'_'.$U3a['Content_id'].'"></textarea></div></div>';
					break;
					case "1":
						$CMS_Deep_Grab2a="SELECT * FROM CMS_Forms WHERE id=:CMS_Form_Id";
						$CMS_Deep_Grab2b=$SQL_Con->prepare($CMS_Deep_Grab2a);
						$CMS_Deep_Grab2b->execute([':CMS_Form_Id'=>$U3a['Content_id']]);
						$CMS_Deep_Grab2c=$CMS_Deep_Grab2b->fetch(PDO::FETCH_ASSOC);
						$CMS_Sel_Fo3_data.='<div class="CMS_EDI_Sub_item_In"><div class="CMS_Lo"><div>ID</div><div>'.$U3a['id'].'</div><div>Name</div><div><input type="text" id="CMS_Na_'.$U3a['Page_types_id'].'_'.$U3a['Content_id'].'" value="'.$U3a['name'].'"></div><div>Description</div><div><input type="text" id="CMS_Na_'.$U3a['Page_types_id'].'_'.$U3a['Content_id'].'" value="'.$U3a['description'].'"></div><div>Location</div><div><input type="text" id="CMS_Na_'.$U3a['Page_types_id'].'_'.$U3a['Content_id'].'" value="'.$U3a['location'].'"></div><div>Created on</div><div>'.$U3a['created_on'].'</div></div>';
					break;
				}
