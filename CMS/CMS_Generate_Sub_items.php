...
$CMS_Content1b->execute([':CMS_P_id' => $CMS_Rights3['id']]);
...
<?PHP		$CMS_Content4a = "SELECT * FROM CMS_Form_fields WHERE form_id = :CMS_C_id";
			...
			foreach($CMS_Content4c as $CMS_L2){
				if(isset($CMS_L2['field_label'])){?><label for="CMS_I_<?PHP echo $CMS_L2['field_key']; ?>"><?PHP echo $CMS_L2['field_label']; ?></label><?PHP } 
				switch($CMS_L2['field_type']){
					case "text": case "email": case "checkbox": case "radio": ?>
					<input type="<?PHP echo $CMS_L2['field_type']; ?>" id="CMS_I_<?PHP echo $CMS_L2['field_anker']; ?>" name="CMS_I_<?PHP echo $CMS_L2['field_key']; ?>" placeholder="<?PHP echo $CMS_L2['field_placeholder']; ?>" />
<?PHP				break;
					case "textarea":  ?>
					<textarea id="CMS_I_<?PHP echo $CMS_L2['field_anker']; ?>" name="CMS_I_<?PHP echo $CMS_L2['field_key']; ?>" placeholder="<?PHP echo $CMS_L2['field_placeholder']; ?>"></textarea>
<?PHP				break;
					case "select":
					echo "Not implemented";
					break;
					case "button": ?>
					<button type="submit" id="CMS_I_<?PHP echo $CMS_L2['field_anker']; ?>"><?PHP echo $CMS_L2['field_placeholder']; ?></button>
<?PHP				break;
				}			
			}?>
			</form>
