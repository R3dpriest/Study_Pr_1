...
/*	print_r($_POST);
	...
	Echo $Usr1 . "-" . $Mai1 . "-" . $Pw11 . "-" . $Pw21;
	echo strlen($Pw11) . "??" . strlen($Pw21) . "??" . strlen($Usr1) . "??" . strlen($Mai1);*/
	...
	if(!strlen($Pw11) == 0 || !strlen($Pw21) == 0 || !strlen($Usr1) == 0 || !strlen($Mai1) == 0) {
		...
		if($CMS_Qry1_count !== 0){array_push($Error, "A user with this name exists"); $E++;  echo "x";}
		if(!preg_match($CMS_Validator->CMS_Email, $Mai1)){ array_push($Error, "Only letters A–Z allowed."); $E++; echo "1";}
		if(!preg_match($CMS_Validator->CMS_PwLength, $Pw11)){ array_push($Error, "Password must be 8–20 characters long."); $E++; echo "2";}
		if(!preg_match($CMS_Validator->CMS_PwLength, $Pw21)){ array_push($Error, "Password must be 8–20 characters long."); $E++; echo "3";}
		if(!preg_match($CMS_Validator->CMS_PwLow, $Pw11)){ array_push($Error, "Password must include at least one lowercase letter"); $E++; echo "4";}
		if(!preg_match($CMS_Validator->CMS_PwLow, $Pw21)){ array_push($Error, "Password must include at least one lowercase letter"); $E++; echo "5";}
		if(!preg_match($CMS_Validator->CMS_PwUp, $Pw11)){ array_push($Error, "Password must include at least one uppercase letter"); $E++; echo "6";}
		if(!preg_match($CMS_Validator->CMS_PwUp, $Pw21)){ array_push($Error, "Password must include at least one uppercase letter"); $E++; echo "7";}
		if(!preg_match($CMS_Validator->CMS_PwDig, $Pw11)){ array_push($Error, "Password must include at least one digit"); $E++; echo "9";}
		if(!preg_match($CMS_Validator->CMS_PwDig, $Pw21)){ array_push($Error, "Password must include at least one digit"); $E++; echo "0";}
		if(!preg_match($CMS_Validator->CMS_PwSpe, $Pw11)){ array_push($Error, "Password must include at least one special character"); $E++; echo "10";}
		if(!preg_match($CMS_Validator->CMS_PwSpe, $Pw21)){ array_push($Error, "Password must include at least one special character"); $E++; echo "11";}
